...
{-
    *** TODO ***
    ...
    -- functia elem primeste un element si o lista si verifica daca elementul
       respectiv se afla in lista intorcant un Bool, insa eu am mai intai lista
...
{-
    - primul pas este inversarea listei = > [scaling 0.5, translation 1 0]
...
bfs start expand = helper [(start, 0)] S.empty
  where
    helper [] _ = []
    helper ((state, dist) : queue) visited
      | state `S.member` visited = helper queue visited
...
regionAvoidingBfs start region = bfs start expand
  where
    expand (x, y) = filter (\p -> not (inside p region)) [(x+1, y), (x-1, y), (x, y+1), (x, y-1)]

-- lista de puncte vecine care este filtrata pta a exclude 
-- punctele care sunt in interiorul regiunii specificate
