...
data RegionShape a
    ...
    deriving (Show, Eq)
-- deriving e metoda care ajuta afisajul pt checker
{-
...
instance Show TransformationAST where
  -- constr de date Translation se acceseaza prin T de la un TransformationShape (constr de tip)
    show (T (Translation tx ty)) = "+(" ++ show tx ++ "," ++ show ty ++ ")"
    show (T (Scaling factor)) = "*<" ++ show factor ++ ">"
    show (T (Combine transformations)) = "[" ++ showTransformations transformations ++ "]"
      where
        showTransformations [] = ""
        showTransformations [t] = show t
        showTransformations (t:ts) = show t ++ "," ++ showTransformations ts -- apelez recursiv
...
instance Show RegionAST where
    show = init . showRegion 0 -- folosesc init ca sa elimin ultimul "\n"
      where
        showRegion :: Int -> RegionAST -> String --fct aux
        --pm pe toti constr lui RegionAST
        showRegion level (R (FromPoints points)) = replicateSpaces level ++ "FromPoints " ++ show points ++ "\n"
        showRegion level (R (Rectangle width height)) = replicateSpaces level ++ "Rectangle " ++ show width ++ " " ++ show height ++ "\n"
        showRegion level (R (Circle radius)) = replicateSpaces level ++ "Circle " ++ show radius ++ "\n"
        showRegion level (R (Complement region)) = replicateSpaces level ++ "~\n" ++ showRegion (level + 1) region
        showRegion level (R (Union region1 region2)) = replicateSpaces level ++ "+\n" ++ showRegion (level + 1) region1 ++ showRegion (level + 1) region2
        showRegion level (R (Intersection region1 region2)) = replicateSpaces level ++ "*\n" ++ showRegion (level + 1) region1 ++ showRegion (level + 1) region2
        showRegion level (R (Transform transformation region)) = replicateSpaces level ++ show transformation ++ "\n" ++ showRegion (level + 1) region

        replicateSpaces :: Int -> String -- fct aux in care ma folosesc de reolicate
        replicateSpaces level = replicate (2 * level) ' ' --lever == nr de tab-uri de exemplu
-- level + 1 pt ca oo apelez rec si vr sa mai dau un tab
...
-- fmap :: (a -> b) -> TransformationShape a -> TransformationShape b
    ...
    fmap f (Combine xs) = Combine (map f xs) --contine campuri de tipul a => d asta aplic f
...
-}
instance Functor RegionShape where -- la fel ca mai sus
    fmap f (Circle x) = Circle x
    fmap f (Rectangle w h) = Rectangle w h
...
{-
    *** TODO ***
    ...
    ?
    -- mai multe transf care contin tipul a si sunt descomp prin foldTransformationAST si
    -- dupa aplic f pe fiecare in parte (f se aplica pe alea care au a)
-}
...
basicTransformationCount = foldTransformationAST combiner
  where
    ...
    combiner (Combine counts) = sum counts --numar cate transf sunt in count
    -- counts -> mai multi de 1, reprezentand Translation sau Sclaing
...
basicEntityCount = foldRegionAST combiner
  where
    combiner :: RegionCombiner (Int, Int) --nr de reg si nr de transf
    combiner (Rectangle _ _) = (1, 0)
    combiner (Circle _) = (1, 0)
    combiner (FromPoints _) = (1, 0)
    combiner (Complement reg) = reg --numar cate reg sau transf sunt in Complement de reg
    combiner (Union reg1 reg2) = ((fst reg1) + (fst reg2), (snd reg1) + (snd reg2))--nr total de regiuni din cele 2 din union si nr total de transformari
    combiner (Intersection reg1 reg2) = ((fst reg1) + (fst reg2), (snd reg1) + (snd reg2))
    combiner (Transform transformation reg) = ((fst reg), basicTransformationCount transformation + (snd reg))
    --pot sa am mai multe transformari la ultima
